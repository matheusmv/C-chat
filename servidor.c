...
void *func_thread_servidor(void *novo_cliente)
{
        Cliente cliente = *(Cliente *)novo_cliente;
...
{
        char *mensagem_enviada = calloc(BUFFER_SIZE, sizeof(char));
        char porta[6];
        ...
        else
        {
                for (int i = 0; i < MAX_CONEXOES; i++)
                {
                        if (clientes[i].cliente_socket != cliente->cliente_socket &&
...
void construir_mensagem(Cliente *cliente_emissor, char *mensagem_buffer)
{
        ...
        time(&horario_atual);
        strncat(horario, ctime(&horario_atual), sizeof(horario) - strlen(horario) - 1);
...
void enviar_mensagem_privada(Cliente *cliente)
{
        ...
        else
        {
                ...
                for (int i = 0; i < MAX_CONEXOES; i++)
                {
                        if (strncmp(clientes[i].usuario, usuario_destino,
                                    strlen(usuario_destino)) == 0)
                        {
                                limpar_buffer_mensagem(cliente->mensagem_cliente, sizeof(cliente->mensagem_cliente));
